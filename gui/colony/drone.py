...
from colony.datatype import Vector3f
from colony.util import wait_while
...
class Drone:
    ...
    def get_position_lla(self) -> Vector3f:
        if self.vehicle is None:
            raise Exception("Vehicle not connected!")
    ...
    def get_position_neu(self) -> Vector3f:
        if self.vehicle is None:
            raise Exception("Vehicle not connected!")
    ...
    def get_position_ned(self) -> Vector3f:
        if self.vehicle is None:
            raise Exception("Vehicle not connected!")
    ...
    def goto(self, waypoint_lla: Vector3f):
        if self.vehicle is None:
            raise Exception("Vehicle not connected!")
    ...
    def comeback(self, mode: str = "SMART_RTL"):
        if self.vehicle is None:
            raise Exception("Vehicle not connected!")
...
if __name__ == "__main__":
    drone = Drone("tcp:127.0.0.1:5782", 0)

    drone.connect()
    # drone.set_gcs_failsafe()
